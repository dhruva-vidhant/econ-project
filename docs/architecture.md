...
| `free_cash_flow_ttm` | Cash flow | Trailing 12mo | Derived (sum of trailing 4 quarters of `free_cash_flow`; annual = annual FCF) | Signed |
| `free_cash_flow_yield` | Valuation (ratio) | Period flow | Derived (`free_cash_flow ÷ market_cap`; quarterly uses TTM FCF) | Signed ratio (×1e6) |
...
| `historical_market_cap` | Valuation | Instant (at period end) | Derived (`close_at_period_end × shares_outstanding_basic`) | Positive |
...
**`free_cash_flow` and `operating_margin`.** Both are multi-input derivations computed at read time for the same supersession-safety reason as `total_debt`. `free_cash_flow = net_income + depreciation_amortization − capital_expenditures` (capital expenditures is stored positive, hence subtracted); all three inputs are required for a period, and capital expenditures uses the full read-time series so the PP&E-roll-forward fallback flows through. `operating_margin = operating_income ÷ revenue` is a dimensionless ratio stored ×1e6 (per the numeric-storage convention below); revenue uses the read-time series (with the bank-revenue fallback), the margin is omitted when revenue is non-positive, and an operating loss yields a valid negative margin. The pure formulas live in the `derived` module (heavily unit-tested in isolation) and the per-period series assembly in `derived::series`, which is parameterized over the repository traits (a `ReadCtx` of `NormalizedFactRepo` / `DerivedMetricRepo` / `HistoricalPriceRepo`) so the IPC handlers and the production-mode integration tests share one code path.

**`historical_market_cap`, `free_cash_flow_ttm`, and `free_cash_flow_yield`.** Market cap is `close_price(period end) × shares_outstanding_basic`, derived at read time: only the external **price** is persisted (in `historical_price`, fetched once at ingest — one close per distinct period end-date, resolved to the nearest prior trading day), while shares come from the period's current primary fact, so a superseded share count can never strand a stale market cap. Trailing-twelve-month free cash flow sums each quarter with its three predecessors (emitted only when the four are strictly consecutive fiscal quarters); for annual periods it is just the annual FCF. Free-cash-flow yield is `free_cash_flow ÷ market_cap` (a ratio ×1e6), using annual FCF on the annual series and TTM FCF on the quarterly series, omitted when market cap is non-positive.
...
- `historical_market_cap` is computed at **read time** as `close_price(period end) × shares_outstanding_basic`. End-of-day closes are fetched once at ingestion (from a market-data adapter — see §7.5) for each distinct period end-date, resolved to the nearest prior trading day, and persisted in `historical_price`; shares come from the period's current primary fact. It is offline-available because the prices are persisted, and supersession-safe because only the price (not the product) is stored. (An earlier draft persisted the product at the filing date; period-end is more comparable across filers and read-time avoids stale sums.)
...
**MarketDataAdapter (price data).** V1 ships with a Yahoo Finance adapter as the default — chosen because it requires no API key and covers FR-050's historical-market-cap requirement out of the box. It calls the public chart endpoint `query1.finance.yahoo.com/v8/finance/chart/{symbol}?period1=&period2=&interval=1d` (the host is already in the app CSP `connect-src` allowlist), parses the daily-close series, and returns closes in USD micro-units; a non-USD `meta.currency` is rejected so a foreign listing can never feed a wrong market cap. SEC share-class dots are mapped to Yahoo hyphens (`BRK.B` → `BRK-B`). The same rate-limit + exponential-backoff conventions as `SecClient` apply. Yahoo's quote endpoints are not contract-backed; the trait exists precisely so the adapter can be swapped without touching call sites if Yahoo's surface shifts (the V18 risk register tracks this).

- **Historical EOD prices** are fetched as a **best-effort** stage at the end of ingestion: the orchestrator collects every distinct period end-date, fetches the daily-close range covering them in one call, and persists one `historical_price` row per end-date — each resolved to the nearest prior trading day so a fiscal close on a weekend/holiday still gets a price. A fetch failure (offline, rate-limited, non-USD, unknown symbol) is recorded as a user-visible warning and leaves financial facts intact; market cap and FCF yield are simply absent until a later refresh succeeds. On refresh the rows are re-upserted (`ON CONFLICT(cik, date)`).
- **Current price** (`current_price`) is available on the adapter for a future live current-market-cap widget; it is not wired into the read path in this revision.
...
|---|---|---|---|
| `free_cash_flow_v1` | `free_cash_flow` | `net_income`, `depreciation_amortization`, `capital_expenditures` | FCF = NI + D&A − CapEx (CapEx is sign-normalized positive at storage). USD micro-units; exact integer arithmetic. **Read-time** derivation; all three inputs required (a missing input omits the period). CapEx uses the read-time series, so the PP&E fallback flows through. |
| `operating_margin_v1` | `operating_margin` | `operating_income`, `revenue` | `OI ÷ Rev`, stored as a ratio ×1e6. **Read-time**; revenue uses the read-time series; omitted when revenue ≤ 0. |
| `free_cash_flow_ttm_v1` | `free_cash_flow_ttm` | `free_cash_flow` (4 trailing quarters) | Sum of a quarter and its three predecessors, emitted only when the four are strictly consecutive fiscal quarters. Annual = annual FCF. **Read-time**. |
| `free_cash_flow_yield_v1` | `free_cash_flow_yield` | `free_cash_flow` (annual) or `free_cash_flow_ttm` (quarterly), `historical_market_cap` | `FCF ÷ market cap`, stored as a ratio ×1e6. **Read-time**; omitted when market cap ≤ 0. |
...
| `historical_market_cap_v1` | `historical_market_cap` | `historical_price[period_end]`, `shares_outstanding_basic` | `close_micro × shares_outstanding` (USD micro-units). **Read-time**: only the close is persisted (in `historical_price`, fetched at ingest — one per period end-date, nearest prior trading day); shares come from the period's current primary fact. Offline-available because prices are persisted; supersession-safe because the product is not. |
